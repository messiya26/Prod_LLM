...
function encodeVerifyToken(token) {
...
function decodeVerifyToken(encoded) {
...
<body style="margin:0;padding:0;background-color:#0a0a0f;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color:#0a0a0f;padding:40px 0;">
    ...
    async sendAdminInvitation(to, inviterName, role, token) {
        const encodedToken = encodeVerifyToken(token);
    ...
    async sendBookingConfirmation(to, name, date, slot, subject) {
        const content = `
      <h1 style="margin:0 0 8px;color:#F5F0E8;font-size:24px;font-weight:700;">
    ...
    async sendRegistrationConfirmation(to, data) {
    ...
    async sendLiveReminder(to, data) {
        const content = `
