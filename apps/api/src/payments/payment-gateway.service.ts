...
export class PaymentGatewayService {
  ...
  async update(id: string, data: { enabled?: boolean; sandboxMode?: boolean; config?: any }) {
  ...
  async getConfigFields(code: string): Promise<{ field: string; label: string; type: string; required: boolean }[]> {
    const fieldMap: Record<string, { field: string; label: string; type: string; required: boolean }[]> = {
      STRIPE: [
